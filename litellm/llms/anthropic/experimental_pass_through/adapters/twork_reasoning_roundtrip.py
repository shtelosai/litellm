...
This module packs reasoning items into the ``signature`` field of a synthetic
Anthropic ``thinking`` content block (prefix-marked), so any Claude-protocol
...
def pack_reasoning_items(items: Any) -> Optional[str]:
    """Pack reasoning items carrying ``encrypted_content`` into a signature string.
...
def unpack_signature(signature: Any) -> Optional[List[Dict[str, Any]]]:
    """Decode a prefix-marked signature back into reasoning items.
